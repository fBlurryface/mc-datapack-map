...
export class TerrainOnlyLayer extends L.GridLayer {
	...
	constructor(options: L.GridLayerOptions) {
		...
		// 比 BiomeLayer(1/4) 更细一点：1/2（256px tile -> 128x128 samples）
		this.calcResolution = 1 / 2
	...
	// === coloring (height + rivers + water + shoreline) ===
	private terrainColor(surface: number, biomeId: string, seaLevel: number, maxY: number): [number, number, number] {
		...
		// rivers: highlight blue (river / frozen_river / modded variants)
		if (id.includes("river")) return [40, 150, 245]

		// shoreline / sea-level band
		if (Math.abs(surface - seaLevel) <= 1.5) return [220, 210, 165]

		// water: deeper => darker
		if (surface <= seaLevel) {
			const t = clamp01((seaLevel - surface) / 32)
			return lerp3([25, 120, 210], [8, 25, 70], t)
		}
	...
	async renderTile(tile: Tile) {
		...
		for (let x = 0; x < samples; x++) {
			for (let z = 0; z < samples; z++) {
				...
				// water wave overlay
				if (Number.isFinite(surface) && surface < seaLevel - 2) {
					tile.ctx.drawImage(
						waveImage,
						(px % 16),
						(pz % 16),
